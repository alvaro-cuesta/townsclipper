...
const decodeSaveString = (string) => {
  ...
  if (foo1.includes('1')) {
    ...
    return `${rest} | ${typeCount} - ${local_84} ${local_8c} - ${foo3} ${foo2} ${foo1}`
  } else {
    const typeCount = bitString.slice(-19, -15)
    const rest = bitString.slice(0, -19)

    return `${rest} | ${typeCount} - ${foo3} ${foo2} ${foo1}`
  }
}
