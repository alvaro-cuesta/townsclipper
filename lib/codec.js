...
const decodeSaveString = (string) => {
  ...
  const foo1Val = parseInt(foo1, 2)

  let rightString = `${foo3} ${foo2} ${foo1}`

  if (foo1Val > 0) {
    const local_8c = read(foo1Val)
    const local_84 = read(foo1Val)
  ...
  const typeCountVal = parseInt(typeCount, 2)

  let typeString

  if (typeCountVal > 0) {
    const types = Array(typeCountVal)
      .fill()
      .map(() => read(4))

    typeString = `${types.join(' ')} (${typeCount})`
  } else {
  ...
  return `${rest} | ${typeString} | ${rightString}`
}
