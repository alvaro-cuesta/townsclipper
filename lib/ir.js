  ...
  while (bitReader.left > 0) {
    ...
    while (true) {
      ...
      if (typeIndex < 0 || typeIndexZeroBased >= typeCount) {
        ...
        throw new Error(`Invalid voxel type ${typeIndexZeroBased}/${typeCount} at (${xPos}, ${yPos}, ${voxels.length + 1}) near bit ${bitReader.cursor}`)
      }

      voxels.push(typeIndexZeroBased)
    }

    if (voxels.length > 0) {
      corner.voxels = voxels
    }
