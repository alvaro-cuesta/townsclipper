...
const bitsToDense = (bits) => {
  const nonBit = bits.match(REGEX_TEST_BITSTRING)

  if (nonBit !== null) {
    throw new Error(`Invalid bit character ${nonBit[0]}`)
  }
...
const denseToBits = (dense, options = {}) => {
  ...
  // Corners
  const cornersWithData = dense.corners
    .filter((corner, i) => i === 0 || corner.hasGround || corner.voxels.length > 0)

  // Ensure there's at least one (empty) corner
  // Townscaper encodes the empty map as AAAE, but without this we'd output AAAA
  const corners = cornersWithData.length > 0
    ? cornersWithData
    : [
        {
          xDelta: null,
