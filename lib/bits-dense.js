...
class BitReader {
  ...
  read(n) {
    ...
    if (bits.length < n) {
      throw new Error(`Could not read ${n} bits (only ${bits.length} remaining) at position ${this.cursor}`)
    }
...
const bitStringToDense = (bitString) => {
  if (bitString.length % 6 !== 0) {
    throw new Error(`Bit string length (${bitString.length}) must be a multiple of 6`)
  }

  const dense = {}
  const bitReader = new BitReader(removePadding(bitString))
  ...
  while (bitReader.remaining > 0) {
    ...
    while (true) {
      ...
      if (corner.voxels.length === MAX_HEIGHT) {
        throw new Error(`Expected end of voxels, reached max height (${MAX_HEIGHT}) near bit ${bitReader.cursor}`)
      }
...
const denseToBitString = (dense, options = {}) => {
  const {
    pretty = false,
    pad = true,
  } = options
